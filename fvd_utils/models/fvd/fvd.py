...
def get_feats(videos, detector, device, bs=10):
    ...
    with torch.no_grad():
        for i in range((len(videos) - 1) // bs + 1):
            batch = [preprocess_single(video) for video in videos[i * bs:(i + 1) * bs]]
            batch_tensor = torch.stack(batch).to(device)  # [B, C, T, H, W]

            # ✅ Explicitly pass x= to fix forward() signature mismatch
            features = detector(x=batch_tensor, **detector_kwargs)
            feats = np.vstack([feats, features.detach().cpu().numpy()])
    return feats
