...
class DVC_ControlNet(LatentDiffusion):
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        if len(batch['flow'])!=0:
            flow= batch['flow']
            flow = flow.to(self.device).to(memory_format=torch.contiguous_format).float()
        ...
        return x, dict(c_crossattn=[c], local_control=[local_conditions], flow_control= [flow] , global_control=[global_conditions])
    ...
    def apply_model(self, x_noisy, t, cond, global_strength=1, *args, **kwargs):
        ...
        if self.mode in ['local', 'uni']:
            ...
            flow_control = torch.cat(cond['flow_control'], 1)
            local_control = self.local_adapter(x=x_noisy, timesteps=t, context=cond_txt, local_conditions=local_control, flow=flow_control)
            local_control = [c * scale for c, scale in zip(local_control, self.local_control_scales)]
            # print(flow_control.shape, local_control.shape)
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, plot_denoise_rows=False,
        ...
        log["flow_control"] = [fz.convert_from_flow(c_flow[0][:2].permute(1, 2, 0).cpu().numpy(), mode='RGB'), 
                               fz.convert_from_flow(c_flow[0][2:].permute(1, 2, 0).cpu().numpy(), mode='RGB')]
        ...
        if sample:
            samples, z_denoise_row = self.sample_log(cond={"local_control": [c_cat], "c_crossattn": [c], "global_control": [c_global], "flow_control":[c_flow]},
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            uc_flow = torch.zeros_like(c_flow)
            uc_full = {"local_control": [uc_cat], "c_crossattn": [uc_cross], "global_control": [uc_global],"flow_control":[uc_flow]}
            samples_cfg, _ = self.sample_log(cond={"local_control": [c_cat], "c_crossattn": [c], "global_control": [c_global],"flow_control":[c_flow]},
                                             batch_size=N, ddim=use_ddim,
                                             ddim_steps=ddim_steps, eta=ddim_eta,
    ...
    def configure_optimizers(self):
        ...
        # Always train adapters if in the appropriate mode
        if self.mode in ['local', 'uni']:
            param_groups.append({"params": self.local_adapter.parameters(), "lr": lr})
            
        if self.mode in ['global', 'uni']:
            param_groups.append({"params": self.global_adapter.parameters(), "lr": lr})
