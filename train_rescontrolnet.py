...
def log_validation(
    vae, text_encoder, tokenizer, unet, controlnet, args, accelerator, weight_dtype, step, base, is_final_validation=False
):
    ...
    if not is_final_validation:
        ...
        controlnet = DualFlowControlNet.from_pretrained(args.output_dir, torch_dtype=weight_dtype)

    scheduler = UniPCMultistepScheduler.from_pretrained(base, subfolder="scheduler")
    pipeline = StableDiffusionDualFlowControlNetPipeline(
                vae=vae,
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Add perceptual weight training",
    )
    parser.add_argument(
        "--edge_weight",
        type=float,
        default=None,
        help="Add edge weight training",
    )
    parser.add_argument(
        "--scale_lr",
        action="store_true",
...
class TransformedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Convert to float32 and permute to CHW
        control_hint = torch.from_numpy(ex["local_conditions"].astype(np.float32)).permute(2, 0, 1)
        flow_hint = torch.from_numpy(ex["flow"].astype(np.float32))
        input_ids = self.tokenize_captions(ex)

        return {
            "flow_hint": flow_hint,
            "control_hint": control_hint,
            "input_ids": input_ids,
            "warped": ex['warped_image'],
            'pixel_values': ex['residual']
        }
...
def collate_fn(examples):
    ...
    conditioning_pixel_values = torch.stack([example["control_hint"] for example in examples])
    conditioning_pixel_values = conditioning_pixel_values.to(memory_format=torch.contiguous_format).float()
    conditioning_flow_values = torch.stack([example["flow_hint"] for example in examples])
    conditioning_flow_values = conditioning_flow_values.to(memory_format=torch.contiguous_format).float()
    conditioning_warped_values = torch.stack([example["warped"] for example in examples])
    # print(examples[0]['input_ids'].shape)
    ...
    return {
        ...
        "input_ids": input_ids,
        "conditioning_flow_values": conditioning_flow_values,
        "conditioning_warped_values": conditioning_warped_values,
    }
...
def main(args):
    ...
    controlnet = ResControlNet(block_out_channels=tuple(unet.config.block_out_channels),  # (320, 640, 1280, 1280)
                               layers_per_block=2,
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            while len(models) > 0:
                ...
                load_model = ResControlNet.from_pretrained(input_dir, subfolder="controlnet")
    ...
    train_dataset = UniDataset(anno_path='data/final_captions.txt',index_file= 'data/index_file_vll5.txt',
                     local_type_list= ['r1','r2', 'flow','flow_b'], resolution=512)
    
    train_dataset = ResidueDataset(train_dataset)
    ...
    if args.perceptual_weight is not None:
         with torch.no_grad(): 
            lpips_model = NormFixLPIPS(net='alex',lpips=True).eval()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                flow_cond = batch["conditioning_flow_values"].to(dtype=weight_dtype)
                warp_cond = batch["conditioning_warped_values"].to(dtype=weight_dtype)
            ...
            logs.update({"loss_mse": loss_mse.detach().item(), 
