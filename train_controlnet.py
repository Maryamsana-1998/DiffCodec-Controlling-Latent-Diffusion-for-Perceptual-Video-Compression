...
class TransformedDataset(Dataset):
    ...
    def tokenize_captions(self, examples, is_train=True):
        ...
        # Force correct batch size: (batch_size, seq_len)
        #print(examples['txt'], inputs.input_ids.shape)
    ...
    def __getitem__(self, idx):
        ...
        pixel_values = torch.from_numpy(ex["jpg"].astype(np.float32)).permute(2, 0, 1)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            while len(models) > 0:
                ...
                load_model = DualFlowControlNet.from_pretrained(input_dir, subfolder="controlnet")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                img_gt = batch["pixel_values"].to(dtype=weight_dtype)
