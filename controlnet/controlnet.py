...
class ControlNetConditioningEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.pre_extraction_fwd = nn.Sequential(
            nn.Conv2d(3, block_out_channels[0], kernel_size=3, padding=1),
            nn.SiLU(),
            nn.Conv2d(block_out_channels[0], block_out_channels[0], kernel_size=3, padding=1,stride=2),
            nn.SiLU(),
        )

        self.pre_extraction_bwd = nn.Sequential(
            nn.Conv2d(3, block_out_channels[0], kernel_size=3, padding=1),
            nn.SiLU(),
            nn.Conv2d(block_out_channels[0], block_out_channels[0], kernel_size=3, padding=1,stride=2),
            nn.SiLU(),
        ...
        self.warper = FeatureWarperSoftsplat(with_learnable_metric=False,in_channels=16)

        self.blocks = nn.ModuleList()
        for i in range(len(block_out_channels) - 2):
            channel_in  = block_out_channels[i + 1]
            channel_out = block_out_channels[i + 2]
    ...
    def forward(self, conditioning,flow_conditioning):

        fwd_flow, bwd_flow = torch.split(flow_conditioning, 2, dim=1)
        fwd_cond, bwd_cond = torch.split(conditioning, 3, dim=1)

        fwd_emb = self.pre_extraction_fwd(fwd_cond)   # [ B, 16, 256, 256 ]
        bwd_emb = self.pre_extraction_bwd(bwd_cond)     # [ B, 16, 256, 256 ]

        fwd_emb = self.warper(fwd_emb,fwd_flow)          # [ B, 16, 256, 256 ]
        bwd_emb = self.warper(bwd_emb,bwd_flow)            # [ B, 16, 256, 256 ]

        embedding = torch.cat([fwd_emb, bwd_emb], dim=1) # [ B, 32,256,256 ]
...
class ControlNetModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        controlnet_cond = self.controlnet_cond_embedding(controlnet_cond,flow_cond)
